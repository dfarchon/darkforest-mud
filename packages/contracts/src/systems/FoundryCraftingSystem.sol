...
contract FoundryCraftingSystem is BaseSystem {
  ...
  function _validateCrafting(
    ...
    // Validate comprehensive material costs with crafting multiplier
    uint256 craftingMultiplier = _getCraftingMultiplier(craftingData.count);
    _validateComprehensiveMaterialCosts(foundry, spaceshipType, materials, amounts, craftingMultiplier);
  }
  ...
  // Comprehensive material cost validation
  function _validateComprehensiveMaterialCosts(
    Planet memory foundry,
    uint8 spaceshipType,
    MaterialType[] memory materials,
    uint256[] memory amounts,
    ...
    // Get required material costs for this spaceship type
    uint256[12] memory requiredCosts = _getSpaceshipMaterialCosts(spaceshipType);

    // Apply crafting multiplier to all required costs
    for (uint256 i = 0; i < 12; i++) {
      requiredCosts[i] = (requiredCosts[i] * craftingMultiplier) / 100;
    }

    // Validate that all required materials are provided with correct amounts
    for (uint256 i = 0; i < materials.length; i++) {
      uint8 materialId = uint8(materials[i]);
      if (materialId == 0 || materialId > 11) revert Errors.UnknownPlanetType(); // Invalid material type

      uint256 requiredAmount = requiredCosts[materialId]; // Convert to 0-based index
      if (amounts[i] != requiredAmount) revert Errors.NotEnoughMaterial();

      // Check if planet has enough of this material
      uint256 currentAmount = foundry.getMaterial(materials[i]);
      if (currentAmount < requiredAmount) revert Errors.InsufficientMaterialOnPlanet();
    }

    // Validate that all required materials are present (not just the ones provided)
    for (uint8 materialId = 1; materialId <= 11; materialId++) {
      uint256 requiredAmount = requiredCosts[materialId];
      if (requiredAmount > 0) {
        // Check if this material was provided in the materials array
        bool materialProvided = false;
        for (uint256 i = 0; i < materials.length; i++) {
          if (uint8(materials[i]) == materialId) {
            materialProvided = true;
            break;
          }
        }
        if (!materialProvided) revert Errors.NotEnoughMaterial1();
      }
    }
  }

  // Get material costs for each spaceship type
  function _getSpaceshipMaterialCosts(uint8 spaceshipType) internal pure returns (uint256[12] memory costs) {
    ...
    if (spaceshipType == 1) {
      // Scout: Water(50), Living Wood(50), Windsteel(100), Aurorium(50)
      costs[uint8(MaterialType.WATER_CRYSTALS)] = 50; // Water
      costs[uint8(MaterialType.LIVING_WOOD)] = 50; // Living Wood
      costs[uint8(MaterialType.WINDSTEEL)] = 100; // Windsteel
      costs[uint8(MaterialType.AURORIUM)] = 50; // Aurorium
    } else if (spaceshipType == 2) {
      // Fighter: Windsteel(200), Aurorium(100), Water(100), Living Wood(100), Pyrosteel(100), Scrapium(100), Sandglass(100), Cryostone(150)
      costs[uint8(MaterialType.WATER_CRYSTALS)] = 100; // Water
      costs[uint8(MaterialType.LIVING_WOOD)] = 100; // Living Wood
      costs[uint8(MaterialType.WINDSTEEL)] = 200; // Windsteel
      costs[uint8(MaterialType.AURORIUM)] = 100; // Aurorium
      costs[uint8(MaterialType.SANDGLASS)] = 100; // Sandglass
      costs[uint8(MaterialType.CRYOSTONE)] = 150; // Cryostone
      costs[uint8(MaterialType.SCRAPIUM)] = 100; // Scrapium
      costs[uint8(MaterialType.PYROSTEEL)] = 100; // Pyrosteel
    } else if (spaceshipType == 3) {
      // Destroyer: All materials with higher costs
      costs[uint8(MaterialType.WATER_CRYSTALS)] = 300; // Water
      costs[uint8(MaterialType.LIVING_WOOD)] = 300; // Living Wood
      costs[uint8(MaterialType.WINDSTEEL)] = 400; // Windsteel
      costs[uint8(MaterialType.AURORIUM)] = 300; // Aurorium
      costs[uint8(MaterialType.MYCELIUM)] = 300; // Mycelium (not used)
      costs[uint8(MaterialType.SANDGLASS)] = 500; // Sandglass
      costs[uint8(MaterialType.CRYOSTONE)] = 500; // Cryostone
      costs[uint8(MaterialType.SCRAPIUM)] = 500; // Scrapium
      costs[uint8(MaterialType.PYROSTEEL)] = 500; // Pyrosteel
      costs[uint8(MaterialType.BLACKALLOY)] = 200; // Blackalloy
      costs[uint8(MaterialType.CORRUPTED_CRYSTAL)] = 200; // Corrupted Crystal
    } else if (spaceshipType == 4) {
      // Carrier: All materials with highest costs
      costs[uint8(MaterialType.WATER_CRYSTALS)] = 500; // Water
      costs[uint8(MaterialType.LIVING_WOOD)] = 500; // Living Wood
      costs[uint8(MaterialType.WINDSTEEL)] = 800; // Windsteel
      costs[uint8(MaterialType.AURORIUM)] = 800; // Aurorium
      costs[uint8(MaterialType.MYCELIUM)] = 500; // Mycelium (not used)
      costs[uint8(MaterialType.SANDGLASS)] = 700; // Sandglass
      costs[uint8(MaterialType.CRYOSTONE)] = 700; // Cryostone
      costs[uint8(MaterialType.SCRAPIUM)] = 700; // Scrapium
      costs[uint8(MaterialType.PYROSTEEL)] = 700; // Pyrosteel
      costs[uint8(MaterialType.BLACKALLOY)] = 400; // Blackalloy
      costs[uint8(MaterialType.CORRUPTED_CRYSTAL)] = 400; // Corrupted Crystal
    } else {
      revert Errors.UnknownPlanetType(); // Invalid spaceship type
    }
  }
}
