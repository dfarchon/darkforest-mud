...
import { ArtifactStatus, ArtifactRarity, PlanetFlagType } from "../../../codegen/common.sol";
...
import { RevealedPlanet, RevealedPlanetData, Artifact as ArtifactTable, Ticker, PlanetFlags } from "../../../codegen/index.sol";
...
import { Flags, FlagsLib } from "../../../lib/Flags.sol";
...
contract PinkBombSystem is ArtifactProxySystem {
  ...
  error PinkBombAlreadyDestroyed(); // 0x0eae5207
  ...
  function destroy(uint256 bombId, Proof memory proof, RevealInput memory input) public {
    ...
    // destroy planet
    Flags flags = Flags.wrap(PlanetFlags.get(bytes32(input.planetHash)));
    if (FlagsLib.check(flags, PlanetFlagType.DESTROYED)) {
      revert PinkBombAlreadyDestroyed();
    }
    flags = FlagsLib.set(flags, PlanetFlagType.DESTROYED);
    PlanetFlags.set(bytes32(input.planetHash), Flags.unwrap(flags));
  }
