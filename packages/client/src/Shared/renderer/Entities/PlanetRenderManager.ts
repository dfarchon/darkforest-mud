...
import { formatNumber, getRange, hasOwner, isLocatable, isSpaceShip } from "@df/gamelogic";
import {
...
export class PlanetRenderManager implements PlanetRenderManagerType {
  ...
  queueLocation(
    ...
    if (!disableHats && planet.canShow) {
      ...
      if (activatedAvatar) {
        // artifact image
        this.queueArtifactImage(planet.location.coords, renderInfo.radii.radiusWorld * 2, activatedAvatar);
        ...
        // normal hat
        this.queueHat(planet, planet.location.coords, renderInfo.radii.radiusWorld, planet.hatType, planet.hatLevel);
      }
    }
  ...
  queueLogoImage(center: WorldCoords, radius: number, hatType: number, hatLevel: number, ifAdminSet: boolean) {
  ...
  private queuePlanetEnergyText(planet: Planet, center: WorldCoords, radius: number, alpha: number) {
    ...
    if (moveHereInProgress && myAtk && toPlanet) {
      let atkString = "";
      if (uiManager.isOwnedByMe(planet) || uiManager.isOwnedByPlayerInMyUnion(planet) || planet.energy === 0) {
