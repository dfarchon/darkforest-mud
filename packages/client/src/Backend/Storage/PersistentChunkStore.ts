...
class PersistentChunkStore implements ChunkStore {
  ...
  constructor({ db, account, contractAddress }: PersistentChunkStoreConfig) {
    ...
    this.throttledSaveChunkCacheToDisk = throttle(
  ...
  public async getHomeLocations(): Promise<WorldLocation[]> {
    ...
    const localStorageHomeLocationsKey = `${this.contractAddress}-${this.account}-homeLocations`;
    const backupData = localStorage.getItem(localStorageHomeLocationsKey);
    if (backupData) {
  ...
  public async saveTouchedPlanetIds(ids: LocationId[]) {
    await this.setKey("touchedPlanetIds", JSON.stringify(ids));
  }

  public async saveRevealedCoords(revealedCoordTups: RevealedCoords[]) {
    await this.setKey("revealedPlanetIds", JSON.stringify(revealedCoordTups));
  }

  public async saveClaimedCoords(claimedCoordTupps: ClaimedCoords[]) {
    await this.setKey("claimedPlanetIds", JSON.stringify(claimedCoordTupps));
  }
