...
import { getBurnerWallet, getNetworkConfig } from "@mud/getNetworkConfig";
...
export const SessionWalletManager: React.FC<{ onClose: () => void }> = ({
  onClose,
}) => {
  const burnerWalletPrivateKey = getBurnerWallet();
  const burnerAccount = createBurnerAccount(burnerWalletPrivateKey as Hex);
  ...
  return (
    <div className="fixed right-5 top-[80px] z-50 flex items-center justify-center bg-black">
      <div
        id="session-wallet-modal"
        className="relative w-full max-w-lg rounded-lg bg-black p-6 shadow-lg"
      >
        <button
          onClick={onClose}
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <span>Main :</span>
            <span>
              {formatAddress(walletClient?.account?.address as `0x${string}`)}
            </span>
            <span>
              {mainWalletBalance
          ...
          <div className="flex items-center justify-between">
            <span>Session :</span>
            <span>{formatAddress(burnerWalletClient?.account?.address)}</span>

            <span>
              {burnerBalance
                ? Number(formatEther(burnerBalance)).toFixed(4)
                : "0"}{" "}
              ETH
            </span>
          </div>
          ...
          {burnerBalanceValue <= LOW_BALANCE_THRESHOLD && (
            <div className="flex items-center justify-center text-red-500">
              Warning: Low session wallet balance!
            </div>
          )}
          {txSuccessful && (
